      ...
      steps {
        dir('./server') {
          withCredentials([file(credentialsId: 'trello-server-env', variable: 'TRELLO_SERVER_ENV')]) {
            bat(script: """copy ${TRELLO_SERVER_ENV} .server\\.env""", label: "create .env file")
          }
        }
    ...
    stage('deploy') {
      ...
      steps {
        withCredentials([usernamePassword(credentialsId: 'dockerhub_credentials', passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_USERNAME')]) {
          ...
          sh(script: """ sudo su ${appUser} """, label: "change to user deploy project") 
          sh(script: """ docker login -u $DOCKER_USERNAME -p $DOCKER_PASSWORD """, label: "login to dockerhub")
          sh(script: 'bash -c "cd ${folderDeploy} && sudo docker-compose pull && sudo docker-compose down && sudo docker-compose up -d"', label: "run project container")
        }
      }
    }
  }
